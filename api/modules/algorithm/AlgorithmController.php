...
class AlgorithmController
{
    ...
    public function continue(Request $req, Response $res): void
    {
        ...
        if ($partidaIdRaw === null) {
            $res::json(['error' => 'ID de partida requerido'], 400);
            return;
        }

        try {
            $result = $this->service->continueGame((string) $partidaIdRaw);

            // Mapear respuesta similar a step
            $esFinal = isset($result['resultado']);

            $salida = [
                'partida_id' => $partidaIdRaw,
                'es_final' => $esFinal,
                'preguntas_respondidas' => ($result['progreso']['paso'] ?? 1) - 1,
                'probabilidad' => ($result['confianza'] ?? 0) / 100.0,
                'personajes_posibles' => $result['candidates'] ?? [],
            ];
            ...
            error_log("Error en AlgorithmController::continue: " . $e->getMessage());
            $res::json(['error' => $e->getMessage()], 500);
        }
    }
}
